...
resource "google_sql_database_instance" "iq_ha_db" {
  ...
  name             = "nexus-iq-ha-db-${random_string.suffix.result}"
  database_version = var.postgres_version
  region          = var.gcp_region
  project         = var.gcp_project_id
  ...
  settings {
    tier                        = var.db_ha_instance_tier
    availability_type          = "REGIONAL"  # Force HA for IQ-HA
    disk_type                  = "PD_SSD"
    disk_size                  = var.db_ha_disk_size
    disk_autoresize           = true
    disk_autoresize_limit     = var.db_ha_max_disk_size
    ...
    database_flags {
      name  = "max_connections"
      value = var.db_ha_max_connections
    }
...
resource "google_sql_database" "iq_ha_database" {
  count    = var.enable_ha ? 1 : 0
  name     = var.db_name
  instance = google_sql_database_instance.iq_ha_db[0].name
  project  = var.gcp_project_id
}
...
# HA Secret Manager for database credentials
resource "google_secret_manager_secret" "db_ha_credentials" {
  count     = var.enable_ha ? 1 : 0
  secret_id = "nexus-iq-ha-db-credentials"
  project   = var.gcp_project_id
...
resource "google_secret_manager_secret_version" "db_ha_credentials" {
  count  = var.enable_ha ? 1 : 0
  secret = google_secret_manager_secret.db_ha_credentials[0].id
  secret_data = jsonencode({
    username = var.db_username
    password = var.db_password
    host     = google_sql_database_instance.iq_ha_db[0].private_ip_address
    port     = 5432
    database = var.db_name
  })
}

# Read Replica for enhanced performance (Optional)
resource "google_sql_database_instance" "iq_read_replica" {
  count               = var.enable_read_replica ? 1 : 0
  name               = "nexus-iq-read-replica-${random_string.suffix.result}"
  database_version   = var.postgres_version
  region            = var.gcp_region
  project           = var.gcp_project_id
  ...
  settings {
    ...
    user_labels = {
      environment = var.environment
      component   = "nexus-iq-read-replica"
    }
...
resource "google_sql_ssl_cert" "iq_ha_client_cert" {
  count       = var.enable_ha ? 1 : 0
  common_name = "nexus-iq-ha-client-cert"
  instance    = google_sql_database_instance.iq_ha_db[0].name
  project     = var.gcp_project_id
}